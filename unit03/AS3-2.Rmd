...
mean(BoeingStock$StockPrice)
mean(CocaColaStock$StockPrice)
mean(GEStock$StockPrice)
mean(ProcterGambleStock$StockPrice)
#IBMStock=144.4,BoeingStock=46.59,CocaColaStock=60.02973,GEStock=59.3035,ProcterGambleStock=77.70452
```
...
```{r}
summary(BoeingStock$StockPrice)
#44.88
```
...
```{r}
ggplot(data=CocaColaStock) +   
    
    # 散布圖對應的函式是geom_point()
    geom_line(aes(x=year,  # 用aes()，描繪散布圖內的各種屬性
                   y=price,
                   main="Scatter Plot of price-year",
                   ) 
               )
plot(x=CocaColaStock$Date, y=CocaColaStock$StockPrice, main="price to year",
