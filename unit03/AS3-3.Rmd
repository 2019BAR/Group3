...
```{r}
CPSData <- read.csv("C:/Users/Alonzo_2/Desktop/R/unit02/data/CPSData.csv")
MetroAreaCodes <- read.csv("C:/Users/Alonzo_2/Desktop/R/unit02/data/MetroAreaCodes.csv")
CountryCodes <- read.csv("C:/Users/Alonzo_2/Desktop/R/unit02/data/CountryCodes.csv")
nrow(CPSData)
#131302
```
...
```{r}
sum(CPSData$CountryOfBirthCode== 57)
#115063
```
...
```{r}
summary(is.na.data.frame(CPSData))
#MetroAreaCode, Married, Education, EmpolymentStatus, Industry
```
...
#2 states, Alaska and Wyoming
```

How many states had all interviewees living in a metropolitan area? Again, treat the District of Columbia as a state.
```{r}
table(is.na(CPSData$MetroAreaCode),CPSData$State)
#3 states, District of Columbia and New Jersey and Rhode island
```
...
```{r}
merge(CPSData, MetroAreaCodes, by.x="MetroAreaCode", by.y="Code", all.x=TRUE)
#MetroArea
```
...
```{r}
table(CPSData$MetroAreaCode) %>% sort()
subset(MetroAreaCodes ,MetroAreaCodes$Code==35620)
#New York-Northern New Jersey-Long Island, NY-NJ-PA
```

__§ 3.4__ Which metropolitan area has the highest proportion of interviewees of Hispanic ethnicity?
```{r}
tapply(CPSData$Hispanic,CPSData$MetroAreaCode,  mean) %>% sort()
subset(MetroAreaCodes, MetroAreaCodes$Code==29700)
#Laredo, TX
```
...
```{r}
godtone <- data.frame(tapply(CPSData$Race== "Asian",CPSData$MetroAreaCode,  mean)) 
subset(godtone, godtone$tapply.CPSData.Race.....Asian...CPSData.MetroAreaCode..mean.>0.2)
#4
```
...
```{r}
tapply(CPSData$Education=="No high school diploma", CPSData$MetroAreaCode, mean, na.rm=T) %>% sort()
subset(MetroAreaCodes, MetroAreaCodes$Code==26980)
#Iowa City, IA
```
...
```{r}
merge(CPSData, CountryCodes, by.x = "CountryOfBirthCode", by.y = "Code", all.x = T)
#CountryOfBirth
```
...
```{r}
table(CPSData$Region!="North", CPSData$CountryOfBirthCode)
subset(CountryCodes, CountryCodes$Code==57)
#United States
```
...
```{r}
subset(MetroAreaCodes, MetroAreaCodes$MetroArea=="New York-Northern New Jersey-Long Island, NY-NJ-PA")
subset(CPSData, CPSData$MetroAreaCode==35620 & CPSData$CountryOfBirthCode!=57)
subset(CPSData, CPSData$MetroAreaCode==35620)
1673/5409
#1673/5409=0.3093
```
...
```{r}
subset(CountryCodes, CountryCodes$Country=="India")
tapply(CPSData$CountryOfBirthCode==210,CPSData$MetroAreaCode, sum) %>% sort %>% tail
subset(MetroAreaCodes, MetroAreaCodes$Code==35620)
#New York-Northern New Jersey-Long Island, NY-NJ-PA
```
...
In Brazil?
```{r}
subset(CountryCodes, CountryCodes$Country=="Brazil")
tapply(CPSData$CountryOfBirthCode==362,CPSData$MetroAreaCode, sum) %>% sort %>% tail
subset(MetroAreaCodes, MetroAreaCodes$Code==71650)
#Boston-Cambridge-Quincy, MA-NH
```

In Somalia?
```{r}
subset(CountryCodes, CountryCodes$Country=="Somalia")
tapply(CPSData$CountryOfBirthCode==448,CPSData$MetroAreaCode, sum) %>% sort %>% tail
subset(MetroAreaCodes, MetroAreaCodes$Code==33460)
