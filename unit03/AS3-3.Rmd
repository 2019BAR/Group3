...
載入資料
```{r}
Country <- read.csv("data/CountryCodes.csv",stringsAsFactors = F)
Metro <- read.csv("data/MetroAreaCodes.csv",stringsAsFactors = F)
...
table(CPS$Industry) %>% sort(decr=T) %>% head()
```
...
```{r}
#0.9422
mean(CPS$Citizenship=="Citizen, Native"|CPS$Citizenship=="Citizen, Naturalized")
CPS$Citizenship %>% table() %>% prop.table()
```
